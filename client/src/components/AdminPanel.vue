...
const triggerUserCSVInput = () => {
  if (userCsvInput.value) userCsvInput.value.click();
};
...
const generateWithAI = async () => {
  ...
  try {
    ...
    if (!res.ok) {
      const errData = await res.json();
      throw new Error(errData.detail || "Erreur lors de la génération par l'IA");
    }
    const data = await res.json();
    ...
    alert("Exercice généré par l'IA avec succès !");
  } catch (err) {
...
const handleFileUpload = async (event) => {
  ...
  reader.onload = async (e) => {
    ...
    if (text.trim().startsWith('[') || text.trim().startsWith('{')) {
      ...
      } catch (err) { console.warn("Échec du parsing JSON."); }
    }
    if (exercisesToImport.length === 0) exercisesToImport = parseExercisesFromText(text);
    if (exercisesToImport.length === 0) { alert("Aucun exercice lisible trouvé."); return; }
    if (!confirm(`${exercisesToImport.length} exercices détectés. Importer ?`)) return;
    const token = localStorage.getItem("access_token");
    let successCount = 0;
    for (const ex of exercisesToImport) {
      ...
      } catch (err) { console.error("Erreur d'import pour", ex.titre, err); }
    }
...
function parseExercisesFromText(text) {
  if (text.includes(';;;')) {
    ...
    for (let i = 0; i + 4 < parts.length; i += 5) {
      results.push({ titre: parts[i], niveau: parts[i + 1], enonce: parts[i + 3], test: parts[i + 4] });
    }
  ...
  blocks.forEach(block => {
    ...
    if (firstLine.length > 3 && firstLine.length < 100) titleStr = firstLine;
    const lowerBlock = block.toLowerCase();
    const testIndex = Math.max(
      lowerBlock.lastIndexOf("test:"), lowerBlock.lastIndexOf("tests:"),
      lowerBlock.lastIndexOf("def test"), lowerBlock.lastIndexOf("try:\n"),
    ...
    results.push({ titre: titleStr, niveau: levelStr, enonce: enonceStr, test: testStr });
  });
  return results;
}
</script>
    ...
    <header class="bg-zinc-50 dark:bg-zinc-800 p-6 flex justify-between items-center shadow-sm">
      <div class="flex items-center gap-8">
        <div class="flex items-center gap-4">
          <div class="w-10 h-10 rounded-2xl bg-blue-600 flex items-center justify-center font-black text-white shadow-lg shadow-blue-500/20">A</div>
        ...
        <main class="flex-1 bg-zinc-50 dark:bg-zinc-900/40 p-8 overflow-y-auto custom-scrollbar transition-colors duration-300">
          <div v-if="!showForm" class="h-full flex flex-col items-center justify-center text-zinc-400 dark:text-zinc-600 transition-opacity animate-in fade-in duration-700">
            <div class="w-24 h-24 rounded-[2.5rem] bg-white dark:bg-zinc-800 flex items-center justify-center mb-8 shadow-xl border border-zinc-100 dark:border-zinc-700/50">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-10 w-10 opacity-30" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
            </div>
          ...
          <div v-else class="max-w-[1400px] px-8 mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20">
            <div class="flex justify-between items-end border-b border-zinc-200 dark:border-zinc-800 pb-6">
              <div>
                ...
                <h1 class="text-3xl font-black text-zinc-800 dark:text-white px-0 tracking-tighter italic">{{ isEditing ? "Modifier l'exercice" : 'Nouvel exercice' }}</h1>
              </div>
              <div class="text-[10px] font-black text-zinc-400 dark:text-zinc-500 bg-white dark:bg-zinc-800 px-4 py-2 rounded-xl border border-zinc-200 dark:border-zinc-700 uppercase tracking-widest shadow-sm">{{ isEditing ? 'ID: ' + formData.id : 'Mode Création' }}</div>
            </div>
            ...
            <div class="space-y-3">
              ...
              <div class="grid grid-cols-2 gap-6 h-[400px]">
                <textarea v-model="formData.enonce" class="w-full h-full bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-[2rem] p-6 font-mono text-sm text-zinc-700 dark:text-zinc-300 focus:ring-2 focus:ring-blue-500 outline-none custom-scrollbar resize-none transition-all shadow-sm" placeholder="Décrivez l'exercice ici..."></textarea>
            ...
            <div class="flex justify-between items-center pt-8 border-t border-zinc-200 dark:border-zinc-800">
              <button @click="generateWithAI" :disabled="isGenerating" class="flex items-center gap-3 px-8 py-4 rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] text-amber-600 dark:text-amber-500 bg-amber-50 dark:bg-amber-500/5 border border-amber-200 dark:border-amber-500/20 hover:bg-amber-100 dark:hover:bg-amber-500/10 transition-all disabled:opacity-30 shadow-sm">
                <svg v-if="isGenerating" class="animate-spin h-4 w-4 text-amber-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
              ...
              <div class="flex gap-4">
                ...
                <button @click="submitForm" class="px-10 py-4 rounded-2xl font-black text-[11px] text-white bg-blue-600 hover:bg-blue-500 transition-all shadow-xl shadow-blue-500/20 uppercase tracking-[0.2em]">{{ isEditing ? 'Sauvegarder' : "Publier l'exercice" }}</button>
              </div>
            </div>
          </div>
        </main>
        ...
        <aside class="w-80 flex-shrink-0 border-r border-zinc-200 dark:border-zinc-800 flex flex-col bg-zinc-50/50 dark:bg-zinc-800/50 transition-colors duration-300">
          <div class="p-6 border-b border-zinc-200 dark:border-zinc-800">
            ...
            <div class="flex gap-2 mb-4">
              ...
              <button @click="triggerUserCSVInput" :disabled="isImportingUsers" class="flex-1 flex items-center justify-center gap-2 py-2.5 px-3 bg-white dark:bg-zinc-800 hover:bg-zinc-100 dark:hover:bg-zinc-700 text-zinc-600 dark:text-zinc-300 rounded-xl font-bold transition-all border border-zinc-200 dark:border-zinc-700 text-[10px] uppercase tracking-wider shadow-sm disabled:opacity-50">
                <svg v-if="isImportingUsers" class="animate-spin h-3 w-3 text-blue-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
          ...
          <div class="flex-1 overflow-y-auto p-4 custom-scrollbar">
            <div v-if="isUsersLoading" class="flex justify-center py-8"><div class="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-600"></div></div>
            <ul v-else class="space-y-2">
              <li v-for="user in filteredUsers" :key="user.username" @click="viewUserDashboard(user.username)" :class="selectedStudent && selectedStudent.username === user.username ? 'bg-blue-600/10 border-blue-500/30' : 'bg-transparent border-transparent hover:bg-white dark:hover:bg-zinc-800/80 hover:shadow-sm'" class="p-3 rounded-2xl border transition-all cursor-pointer flex items-center justify-between group">
                <div class="flex items-center gap-3 overflow-hidden">
                  <div class="w-9 h-9 rounded-xl flex-shrink-0 flex items-center justify-center text-xs font-black transition-all shadow-sm" :class="selectedStudent && selectedStudent.username === user.username ? 'bg-blue-600 text-white' : 'bg-white dark:bg-zinc-900 text-zinc-400 border border-zinc-100 dark:border-zinc-700'">
        ...
        <main class="flex-1 bg-zinc-50 dark:bg-zinc-900/40 p-8 overflow-y-auto custom-scrollbar relative transition-colors duration-300">
          <div v-if="isCreatingUser" class="max-w-4xl mx-auto space-y-8 bg-white dark:bg-zinc-800 p-10 rounded-[2.5rem] shadow-2xl border border-zinc-100 dark:border-zinc-700 transition-all animate-in fade-in zoom-in-95 duration-500">
            ...
            <div class="flex justify-end gap-4 pt-8 border-t border-zinc-100 dark:border-zinc-700">
              ...
              <button @click="submitCreateUserForm" :disabled="isAddingUser" class="px-10 py-4 rounded-xl font-black text-[11px] text-white bg-blue-600 hover:bg-blue-500 transition-all shadow-xl shadow-blue-500/20 uppercase tracking-[0.2em] flex items-center gap-3 disabled:opacity-50">
                <svg v-if="isAddingUser" class="animate-spin h-4 w-4 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
                Créer le compte
              </button>
            </div>
          </div>
          ...
          <div v-else-if="!selectedStudent" class="h-full flex flex-col items-center justify-center text-zinc-400 transition-opacity animate-in fade-in duration-700">
            ...
            <p class="text-xs opacity-60 mt-4 italic text-center max-w-xs font-medium">Sélectionnez un élève pour visualiser ses performances</p>
          </div>
